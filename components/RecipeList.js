  ...
  const navigation = useNavigation();
    ...
    for (let i = 0; i < props.route.params.recipes.length; i++) {
      cards.push({
        ...
        content: 
        <View>
        <Text style={styles.description}><RenderHtml source={{html:props.route.params.description[0].summary} }/></Text>
        <Text style={styles.description}>Preparation Time: {props.route.params.description[0].preparationMinutes}</Text>
        <Text style={styles.description}>Cooking Time: {props.route.params.description[0].cookingMinutes}</Text>
        <Text style={styles.description}><Text style={{fontWeight:'bold'}}>Servings: </Text>{props.route.params.description[0].servings}</Text>
        <TouchableOpacity
        style={{
          borderWidth:1,
              borderColor:'black',
        ...
        onPress={async()=>{
          navigation.navigate("Recipe",{'recipes': props.route.params.recipes[i], "description": props.route.params.description[0]})
